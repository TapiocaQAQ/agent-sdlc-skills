...
### 6. Present the commit for review, then create it

<!-- DELIBERATE DELTA from upstream appleboy/skills: the original ended Step 6 with
     "Run `git commit` directly — do not ask for confirmation" (invoking = authorization).
     This pack intentionally adds a human review gate: show the full message and commit
     only after the user approves. This is a strengthening of the gate, not a copy error. -->
...
If there is a breaking change, include the `BREAKING CHANGE:` footer above the trailer:
...
The `Co-Authored-By: Claude <noreply@anthropic.com>` trailer is **mandatory** — always append it as the last line of every commit message (after the `BREAKING CHANGE:` footer when one is present).

**Review before committing.** Show the user the complete proposed commit message — the full header, summary, and trailer exactly as it will be committed — and wait for their approval. If the user requests edits, revise and show the updated message again. Do **not** run `git commit` until the user has approved.

Once approved, use a HEREDOC to pass the multi-line message to `git commit`:
