...
> **Wiring — this is the PR-time re-classification, using `classify-change`'s rubric.** Apply `classify-change`'s six-question rubric (propagation, change frequency, tech-debt, examples, failure cost, review intensity — stricter rule wins) to the *actual diff*. This is a deliberate second checkpoint over the plan-time classification, which was based on *intent*; this one is based on *evidence*. Don't invent a fresh rubric — re-run the shared one against what the code actually became. If implementation drifted a planned-leaf change into core (it now modifies a shared module it wasn't scoped to), this check must catch it: an **escalation** (leaf → core) is a finding to surface in the PR, never a silent pass; a **de-escalation** (core → leaf) needs human sign-off. The quick diagnostic above is the fast path when the rubric's answer is obvious.
...
- [ ] No secrets AND no real/production data in the diff — API keys, credentials, tokens, real DBs / data dumps / downloaded datasets, PII / real user records. These belong in `.gitignore`, never in version control; if any appears in the diff, stop and remove it before the PR.
